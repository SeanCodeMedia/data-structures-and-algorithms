...
def word_count(sentence):
    ...
    for x in range(len(sentence)):
        if not sentence[x].isspace() and sentence[x].isalpha():
            word_builder += sentence[x].lower()

        elif sentence[x].isspace():
            if word_builder in maps:
                maps[word_builder] = maps[word_builder] + 1
                word_builder = ""
            elif word_builder not in maps:
                maps[word_builder] = 1
                word_builder = ""
...
def character_mode(string):
    ...
    for x in lowercase:
        if x in string_counter:
            string_counter[x] = string_counter[x] + 1
        elif not x.isspace():
            string_counter[x] = 1

    string_builder = ""

    for x in string_counter.keys():

        if string_counter[x] == max_count_value:
            string_builder += x

        elif string_counter[x] > max_count_value:
            max_count_value = string_counter[x]
            string_builder = x
...
def anagram_pair(string1, string2):
    ...
    for x in string1:
        sort_word_one[ord(x) - 97] = sort_word_one[ord(x) - 97] + 1

    for y in string2:
        sort_word_two[ord(y) - 97] = sort_word_two[ord(y) - 97] + 1

    for i, j in zip(sort_word_one, sort_word_two):
